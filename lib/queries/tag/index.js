...
function computeQuery (tags, options) {
  var phrases = computePhrase(tags);
  var idPrefix = options.idPrefix ? "(prefix field=id '" + options.idPrefix + "')" : '';
  return util.format('(and (%s %s)%s)', options.operator, phrases.join(''), idPrefix);
}

function computePhrase (tags) {
  return tags.map(function (tag) {
    if (isArray(tag)) return util.format('(or %)', computePhrase(tag));
