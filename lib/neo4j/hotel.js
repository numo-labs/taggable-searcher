...
function buildQuery () {
  ...
  if (geo.length > 0 && amenities.length === 0) {
    return geo.map(function (g) {
      return util.format('MATCH %s(n%s) RETURN n LIMIT %s', g, label, limit);
    });
  } else if (geo.length === 0 && amenities.length > 0) {
    return util.format('MATCH %s RETURN n LIMIT %s', amenities.map(function (a) {
      return util.format('(n%s)-[:HAS]->(:amenity {id:"%s"})', label, a);
    }), limit);
  } else {
    return geo.map(function (g) {
      return util.format('MATCH %s RETURN n LIMIT %s', amenities.map(function (a) {
        return util.format('(:geo {id:"%s"})<-[LOCATED_IN*..10]-(n%s)-[:HAS]->(:amenity {id:"%s"})', g, label, a);
      }), limit);
    }).join(' UNION ');
  }
}
